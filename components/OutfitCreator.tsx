...
const OutfitSlot: React.FC<{ item?: ClothingItem; icon: React.ReactNode; title: string }> = ({ item, icon, title }) => (
  <div className="bg-gray-100 rounded-lg flex flex-col items-center justify-center p-2 text-center">
    {item ? (
...
const OutfitCreator: React.FC<{ items: ClothingItem[] }> = ({ items }) => {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
        const result = await generateOutfitImage(selectedItems);
  ...
  const canGenerate = !!(outfit.top && outfit.bottom);

  return (
    <div className="animate-fade-in grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-1 bg-white p-6 rounded-lg shadow-md max-h-[80vh] overflow-y-auto">
        <h3 className="text-xl font-bold mb-4 sticky top-0 bg-white py-2">خزانتك</h3>
        {items.length > 0 ? (
